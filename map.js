...
var layers = [
        ...
        ['Food Stamps', 'percentage of household receiving foodstamps'],
        ['less than 2%'],
        ['2%~5%'],
        ['5%~10%'],
        ['10%~15%'],
    	...
    	['$$$ Market and Grocery', 'expensive'],
    	['$$ Market and Grocery', 'affordable'],
    	['$ Market and Grocery', 'cheape']
    ];
...
for (i=0; i<layers.length; i++) {
    ...
    var itemHTML = "<div><span class='legend-key'></span><span>" + layer[0] + "</span><span class='help'>?</span><span class='tooltip'>" + layer[1] + "</span></div>"; 
...
map.on('mousemove', function(e) {   // Event listener to do some code when the mouse moves, see https://www.mapbox.com/mapbox-gl-js/api/#events. 
    ...
    // event listener for slider to change heatmap
    var slider = document.getElementById("myRange");

    // Update the current slider value (each time you drag the slider handle)
    slider.oninput = function() {
        var radius = this.value / 4; 

        // convert radius from miles to pixels
        radius = radius * 100; // change this to a miles value!!!

        map.setPaintProperty('markets-heatmap', 'heatmap-radius', radius);
    }
